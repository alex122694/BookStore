...
import bookstore.book.BookSalesDTO;
import bookstore.book.BooksBean;
import bookstore.book.MonthlySalesDTO;
import bookstore.book.SalesOverviewDTO;
import bookstore.core.BusinessException;
import bookstore.coupon.CouponBean;
import bookstore.coupon.UserCouponBean;
import bookstore.coupon.UserCouponRepository;
import bookstore.user.UserBean;
...
import bookstore.coupon.UserCouponRepository; // Added
import bookstore.coupon.UserCouponBean; // Added
...
public class OrderService {
	...
	public List<BookSalesDTO> getTopSellingBooks(Timestamp start, Timestamp end) {
		StringBuilder jpql = new StringBuilder(
				"SELECT new bookstore.book.BookSalesDTO(b.bookName, SUM(oi.quantity)) " +
						"FROM OrderItem oi " +
						"JOIN oi.booksBean b " +
						"JOIN oi.orders o " +
	...
	public List<BookSalesDTO> getTopSellingBooksFull(Timestamp start, Timestamp end) {
	    StringBuilder jpql = new StringBuilder(
	            "SELECT new bookstore.book.BookSalesDTO(" +
	...
	@Transactional(readOnly = true)
	public List<bookstore.book.MonthlySalesDTO> getRecentSalesTrends() {
		...
		String jpql = "SELECT new bookstore.book.MonthlySalesDTO(" +
		...
		return entityManager.createQuery(jpql, bookstore.book.MonthlySalesDTO.class)
				.setParameter("startDate", startTs)
				.getResultList();
	}

	@Transactional(readOnly = true)
	public bookstore.book.SalesOverviewDTO getSalesOverview(Timestamp start, Timestamp end) {
		// 1. Total Revenue
		...
		return new bookstore.book.SalesOverviewDTO(totalRevenue, totalOrders, totalBooksSold);
	}
